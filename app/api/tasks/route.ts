...
export async function POST(request: Request) {
  try {
    ...
    if (user.role !== "ADMIN") {
      if ("price" in b || "status" in b || "priced" in b) return NextResponse.json({error:"Solo administración asigna precios y estados."},{status:403});
      b.price = 0;
    }
    ...
    const record = await prisma.serviceRecord.findFirst({where:{publicId:b.recordId,deletedAt:null}});
    if (!record) return NextResponse.json({error:"Servicio no encontrado"},{status:404});
    const task = await prisma.serviceTask.create({data:{serviceRecordId:record.id,concept:b.concept.trim(),description:b.description,price:Number(b.price),priced:user.role==="ADMIN"}});
...
export async function PATCH(request: Request) {
  try {
    const user = await requireStaff(request, true);
    const b = await request.json();
    ...
    if(!await prisma.serviceTask.findFirst({where:{id:b.id,serviceRecord:{deletedAt:null}}})) return NextResponse.json({error:"Tarea no encontrada"},{status:404});
    ...
    const data: {status:any;concept?:string;description?:string;price?:number;priced?:boolean} = {status:b.status};
    if (user.role === "ADMIN" && "price" in b) {
      ...
      Object.assign(data,{price:Number(b.price),priced:true,concept:b.concept.trim(),description:b.description});
    }
