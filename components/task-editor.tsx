...
type Task = {id:string;concept:string;description:string|null;price:string;priced:boolean;status:string;images:{id:string;url:string;kind:string}[]};
export default function TaskEditor({recordId,admin,tasks}:{recordId:string;admin:boolean;tasks:Task[]}) {
  ...
  return <div className="space-y-6">
    ...
    {<form className="card space-y-4" onSubmit={async e=>{e.preventDefault();const form=e.currentTarget;const data=Object.fromEntries(new FormData(form));if(await send("/api/tasks","POST",{...data,recordId}))form.reset();}}>
      ...
      {admin&&<label className="block">Precio (MXN)<input className="w-full bg-slate-900 p-3" name="price" type="number" min="0" max="99999999" step="0.01" required /></label>}
    ...
    {tasks.map(task=><section className="card" key={task.id}>
      ...
      </form>:<><h2 className="text-xl font-bold">{task.concept}</h2><p>{task.description}</p><p>{statuses[task.status]}</p></>}
      {!task.priced&&<p className="text-amber-300">Pendiente de cotizar</p>}
      <div className="mt-6 grid gap-5 md:grid-cols-2">{(admin?["BEFORE","AFTER"]:["BEFORE"]).map(kind=><div key={kind}><h3 className="font-bold">{kind==="BEFORE"?"Antes":"Después"}</h3>
